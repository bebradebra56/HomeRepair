...
class HomeRepairVi(
    private val homeRepairContext: Context,
    private val homeRepairCallback: HomeRepairCallBack,
    private val homeRepairWindow: Window
) : WebView(homeRepairContext) {
    private var homeRepairFileChooserHandler: ((ValueCallback<Array<Uri>>?) -> Unit)? = null
    fun homeRepairSetFileChooserHandler(handler: (ValueCallback<Array<Uri>>?) -> Unit) {
        this.homeRepairFileChooserHandler = handler
    }
    init {
        ...
        webSettings.apply {
            ...
            userAgentString = WebSettings.getDefaultUserAgent(homeRepairContext).replace("; wv)", "").replace("Version/4.0 ", "")
        ...
        super.setWebViewClient(object : WebViewClient() {
            override fun shouldOverrideUrlLoading(
                ...
                return if (request?.isRedirect == true) {
                    ...
                    try {
                        homeRepairContext.startActivity(intent)
                    } catch (e: Exception) {
                        Toast.makeText(homeRepairContext, "This application not found", Toast.LENGTH_SHORT).show()
                    }
                    true
                }
            }
            ...
            override fun onPageFinished(view: WebView?, url: String?) {
                ...
                if (url?.contains("https://test-web.syndi-test.net") != true) {
                    HomeRepairApplication.homeRepairInputMode = WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN
                    Log.d(HomeRepairApplication.HOME_REPAIR_MAIN_TAG, "onPageFinished : ${HomeRepairApplication.homeRepairInputMode}")
                    homeRepairWindow.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN);
                    homeRepairEnableStableInputScroll()
                } else {
                    HomeRepairApplication.homeRepairInputMode = WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE
                    Log.d(HomeRepairApplication.HOME_REPAIR_MAIN_TAG, "onPageFinished : ${HomeRepairApplication.homeRepairInputMode}")
                    homeRepairWindow.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE);
                }
            }
        ...
        super.setWebChromeClient(object : WebChromeClient() {
            ...
            override fun onShowFileChooser(
                ...
                homeRepairFileChooserHandler?.invoke(filePathCallback)
                return true
            }
            override fun onCreateWindow(
                ...
                homeRepairHandleCreateWebWindowRequest(resultMsg)
                return true
            }
        })
    }
    ...
    fun homeRepairFLoad(link: String) {
        super.loadUrl(link)
    }

    private fun homeRepairEnableStableInputScroll() {
        evaluateJavascript(
            """
        (function() {
    ...
    private fun homeRepairHandleCreateWebWindowRequest(resultMsg: Message?) {
        if (resultMsg == null) return
        if (resultMsg.obj != null && resultMsg.obj is WebView.WebViewTransport) {
            val transport = resultMsg.obj as WebView.WebViewTransport
            val windowWebView = HomeRepairVi(homeRepairContext, homeRepairCallback, homeRepairWindow)
            transport.webView = windowWebView
            resultMsg.sendToTarget()
            homeRepairCallback.homeRepairHandleCreateWebWindowRequest(windowWebView)
        }
    }
